...
export const STANDARD_CASE_TEMPLATES: CaseTemplate[] = [
  ...
  },
  {
    ...
    description: "Start a spin, force a reload while the round is still resolving, and verify the recovered session honors the round without duplicate debit or lost win.",
    category: "base_game",
    severity: "critical",
    setup_instructions:
    ...
    spin_count: 1,
    allowed_interruptions: ["BIG_WIN_POPUP", "BONUS_POPUP", "FREE_SPIN_TRIGGERED"],
    custom_assertions: [
      { id: "round-captured-after-reload", description: "at least one round result was captured around the reload", check_code: "collector.spins.length >= 1" },
      { id: "no-negative-balance", description: "recovered balance never goes negative", check_code: "collector.spins.every(s => typeof s.endingBalance !== 'number' || s.endingBalance >= -0.01)" },
      { id: "no-duplicate-round-id", description: "reload did not duplicate a settled round id", check_code: "new Set(collector.spins.map(s => s.id).filter(Boolean)).size === collector.spins.map(s => s.id).filter(Boolean).length" },
    ],
  },
  {
    id: "bet-minimum-matches-rule",
    name: "Bet settings — minimum bet matches rule",
    description: "Click bet-minus to the floor and verify the on-screen BET equals the configured minimum bet.",
    category: "bet_boundary",
    severity: "major",
    setup_instructions:
    ...
    spin_count: 0,
    custom_assertions: [
      { id: "screen-bet-is-min", description: "BET display equals {{betMin}} when configured", check_code: "(() => { const expected = {{betMin}}; return expected == null || screen.bet == null || Math.abs(screen.bet - expected) <= 0.01; })()" },
    ],
  },
  {
    id: "bet-maximum-matches-rule",
    name: "Bet settings — maximum bet matches rule",
    description: "Click bet-plus to the ceiling and verify the on-screen BET equals the configured maximum bet.",
    category: "bet_boundary",
    severity: "major",
    setup_instructions:
    ...
    spin_count: 0,
    custom_assertions: [
      { id: "screen-bet-is-max", description: "BET display equals {{betMax}} when configured", check_code: "(() => { const expected = {{betMax}}; return expected == null || screen.bet == null || Math.abs(screen.bet - expected) <= 0.01; })()" },
    ],
  },
  {
    id: "bet-persists-after-spin",
    name: "Bet settings — bet persists after spin",
    description: "Set a non-default bet, spin once, and verify the game keeps the same bet after the round settles.",
    ...
    spin_count: 1,
    custom_assertions: [
      { id: "post-spin-bet-display-matches-spin", description: "BET display after the spin matches the bet used by the captured round", check_code: "(() => { const last = collector.spins[collector.spins.length - 1]; return !last || screen.bet == null || typeof last.betAmount !== 'number' || Math.abs(screen.bet - last.betAmount) <= 0.01; })()" },
      { id: "balance-conservation", description: "balance arithmetic holds at the chosen bet", check_code: "spin.startingBalance == null || Math.abs(spin.endingBalance - (spin.startingBalance - spin.betAmount + spin.winAmount)) <= 0.01" },
    ],
  },
  {
    id: "bet-locked-mid-spin",
    name: "Bet settings — bet controls locked mid-spin",
    description: "Try to change bet while a spin is resolving; verify the round is not disrupted and no extra spin is created.",
    category: "bet_variation",
    severity: "major",
    setup_instructions:
    ...
    spin_count: 1,
    custom_assertions: [
      { id: "single-round-only", description: "mid-spin bet-control clicks did not create extra spin rounds", check_code: "getRoundEndSpins(collector.spins).length === 1" },
      { id: "no-debounced-spin-warning", description: "runner did not detect swallowed/dropped spin clicks", check_code: "warnings.filter(w => /debounced|popup may have blocked|no spin.*response/i.test(w)).length === 0" },
    ],
  },
  {
    id: "spin-blocked-if-bet-exceeds-balance",
    name: "Bet settings — spin blocked if bet exceeds balance",
    description: "When the wallet cannot afford the selected bet, spin must be blocked with no negative balance.",
    category: "bet_boundary",
    severity: "critical",
    setup_instructions:
    ...
    spin_count: 1,
    custom_assertions: [
      { id: "balance-never-negative", description: "no captured balance goes negative", check_code: "collector.spins.every(s => typeof s.endingBalance !== 'number' || s.endingBalance >= -0.01)" },
    ],
  },
  {
    id: "base-balance-never-negative",
    name: "Base game — balance never negative",
    description: "Drive the session toward a low balance and verify spins stop or reconcile without ever going below zero.",
    category: "base_game",
    severity: "critical",
    setup_instructions:
    ...
    spin_count: 20,
    custom_assertions: [
      { id: "balance-never-negative", description: "every captured ending balance is >= 0", check_code: "collector.spins.every(s => typeof s.endingBalance !== 'number' || s.endingBalance >= -0.01)" },
      { id: "rounds-reconcile", description: "all captured rounds reconcile", check_code: FINANCIAL_INVARIANT },
    ],
  },
  {
    id: "max-win-cap-enforced",
    name: "Max win cap — cap enforced when reached",
    description: "Run a long winning-round watch and verify any capped win obeys the game's maximum-win behavior when the cap is observable.",
    ...
    allowed_interruptions: ["BIG_WIN_POPUP", "BONUS_POPUP", "FREE_SPIN_TRIGGERED"],
    custom_assertions: [
      { id: "no-negative-or-invalid-win", description: "all captured wins are finite and non-negative", check_code: "collector.spins.every(s => typeof s.winAmount === 'number' && s.winAmount >= 0)" },
      { id: "rounds-reconcile", description: "all captured rounds reconcile while watching for cap", check_code: FINANCIAL_INVARIANT },
    ],
  },
  {
  ...
  },
  {
    ...
    description: "Run native autoplay at the minimum bet and verify balance integrity across the batch.",
    category: "autoplay",
    severity: "major",
    requiresFeature: "autoSpin",
    setup_instructions:
    ...
    spin_count: 50,
    custom_assertions: [
      { id: "autoplay-captured", description: "autoplay captured at least one settled round", check_code: "getRoundEndSpins(collector.spins).length >= 1" },
      { id: "autoplay-balance", description: "cumulative bet/win reconciles", check_code: FINANCIAL_INVARIANT },
    ],
  },
  {
    ...
    description: "Run native autoplay at the maximum bet when funds allow and verify integrity or a clean stop when funds run out.",
    category: "autoplay",
    severity: "major",
    requiresFeature: "autoSpin",
    setup_instructions:
    ...
    custom_assertions: [
      { id: "autoplay-captured", description: "autoplay captured at least one settled round", check_code: "getRoundEndSpins(collector.spins).length >= 1" },
      { id: "autoplay-balance", description: "cumulative bet/win reconciles", check_code: FINANCIAL_INVARIANT },
      { id: "balance-never-negative", description: "funds exhaustion does not make balance negative", check_code: "collector.spins.every(s => typeof s.endingBalance !== 'number' || s.endingBalance >= -0.01)" },
    ],
  },
  {
    id: "autoplay-turbo-50",
    name: "Autoplay — 50 spins with Turbo enabled",
    description: "Enable Turbo spin, run autoplay, and verify accuracy is unaffected.",
    category: "autoplay",
    severity: "major",
    ...
    spin_count: 50,
    custom_assertions: [
      { id: "autoplay-turbo-balance", description: "cumulative bet/win reconciles with Turbo enabled", check_code: FINANCIAL_INVARIANT },
    ],
  },
  {
    id: "autoplay-quick-spin-50",
    name: "Autoplay — 50 spins with Quick Spin enabled",
    description: "Enable Quick Spin when available, run autoplay, and verify accuracy is unaffected.",
    category: "autoplay",
    severity: "major",
    requiresFeature: ["autoSpin", "turbo"],
    setup_instructions:
    ...
    spin_count: 50,
    custom_assertions: [
      { id: "autoplay-quick-balance", description: "cumulative bet/win reconciles with Quick Spin enabled", check_code: FINANCIAL_INVARIANT },
    ],
  },
  {
    id: "autoplay-manual-stop",
    name: "Autoplay — manual stop works",
    description: "Start a 50-spin autoplay batch, stop after several rounds, and verify it halts cleanly after the current round.",
    category: "autoplay",
    severity: "major",
    requiresFeature: "autoSpin",
    setup_instructions:
    ...
    spin_count: 50,
    custom_assertions: [
      { id: "manual-stop-captured-some", description: "manual stop flow captured at least one round", check_code: "getRoundEndSpins(collector.spins).length >= 1" },
      { id: "manual-stop-reconciles", description: "captured rounds reconcile after manual stop", check_code: FINANCIAL_INVARIANT },
    ],
  },
  {
  ...
  },
  {
    ...
    description: "Toggle Ante ON and verify the stake changes according to the special-bet behavior.",
    category: "special_bet",
    severity: "major",
    requiresFeature: "extraBet",
    setup_instructions:
    ...
    spin_count: 1,
    custom_assertions: [
      { id: "ante-stake-positive", description: "Ante ON produces a valid positive stake", check_code: "typeof spin.betAmount === 'number' && spin.betAmount > 0" },
      { id: "ante-stake-at-least-min", description: "Ante ON stake is not below configured minimum when known", check_code: "(() => { const min = {{betMin}}; return min == null || typeof spin.betAmount !== 'number' || spin.betAmount + 0.01 >= min; })()" },
      { id: "ante-balance", description: "balance arithmetic holds with Ante ON", check_code: "spin.startingBalance == null || Math.abs(spin.endingBalance - (spin.startingBalance - spin.betAmount + spin.winAmount)) <= 0.01" },
    ],
  },
  {
    ...
    description: "Turn Ante ON, then OFF, and verify the base stake is restored.",
    category: "special_bet",
    severity: "major",
    requiresFeature: "extraBet",
    setup_instructions:
    ...
    spin_count: 1,
    custom_assertions: [
      { id: "ante-off-base-bet", description: "Ante OFF spin uses the configured base minimum bet when known", check_code: "(() => { const min = {{betMin}}; return min == null || typeof spin.betAmount !== 'number' || Math.abs(spin.betAmount - min) <= 0.01; })()" },
      { id: "ante-off-balance", description: "balance arithmetic holds after Ante OFF", check_code: "spin.startingBalance == null || Math.abs(spin.endingBalance - (spin.startingBalance - spin.betAmount + spin.winAmount)) <= 0.01" },
    ],
  },
  {
    id: "ante-on-buy-feature-disabled",
    name: "Ante bet — Buy Feature disabled when Ante ON",
    description: "When a title disables Buy Feature under Ante ON, verify the buy control is unavailable or rejected without charge.",
    category: "special_bet",
    severity: "major",
    ...
    spin_count: 0,
    custom_assertions: [
      { id: "no-buy-committed", description: "inspection did not trigger a buy spin", check_code: "collector.spins.length === 0" },
      { id: "no-errors", description: "no engine errors while checking Buy under Ante ON", check_code: "warnings.filter(w => /error|exception|threw/i.test(w)).length === 0" },
    ],
  },
  {
  ...
  },
  {
    ...
    description: "Enter free spins and verify free-spin rounds do not debit the bet; balance only rises by wins.",
    category: "free_spins",
    severity: "critical",
    ...
    custom_assertions: [
      { id: "fs-no-bet-deduction", description: "free-spin rounds do not deduct a bet", check_code: "collector.spins.filter(s => s.isFreeSpin === true).every(s => s.startingBalance == null || s.endingBalance >= s.startingBalance - 0.001)" },
      { id: "fs-rounds-shaped", description: "free-spin rounds are well formed", check_code: "collector.spins.filter(s => s.isFreeSpin === true).every(s => typeof s.id === 'string' && s.id.length > 0 && typeof s.winAmount === 'number' && s.winAmount >= 0)" },
    ],
  },
  {
    id: "buy-feature-trigger-spin-valid",
    name: "Buy feature — bought trigger spin valid",
    description: "Buy the feature and verify the trigger produces a valid bonus/free-spin entry.",
    category: "buy_feature",
    severity: "critical",
    requiresFeature: "buyBonus",
    setup_instructions:
    ...
    allowed_interruptions: ["FREE_SPIN_TRIGGERED", "BIG_WIN_POPUP", "BUY_FEATURE_POPUP"],
    custom_assertions: [
      { id: "buy-deduction-observed", description: "buy trigger has a large purchase deduction", check_code: "(() => { const d = detectBuyFeatureDeduction(collector.spins, 0, balanceBefore); return d != null && d.ratio >= 20; })()" },
      { id: "feature-state-observed", description: "feature/free-spin frames are observed after purchase", check_code: "collector.spins.some(s => s.isFreeSpin === true || /FREE|BONUS/i.test(String(s.state || '')))" },
    ],
  },
  {
    id: "buy-feature-no-bet-deducted-during-feature",
    name: "Buy feature — no bet deducted during bought feature",
    description: "After buying a feature, verify feature/free-spin rounds do not charge an additional per-spin bet.",
    category: "buy_feature",
    severity: "critical",
    requiresFeature: "buyBonus",
    setup_instructions:
    ...
    allowed_interruptions: ["FREE_SPIN_TRIGGERED", "BIG_WIN_POPUP", "BUY_FEATURE_POPUP"],
    custom_assertions: [
      { id: "bought-fs-no-per-spin-deduction", description: "free-spin/bought-feature rounds do not deduct per-spin bet", check_code: "collector.spins.filter(s => s.isFreeSpin === true).every(s => s.startingBalance == null || s.endingBalance >= s.startingBalance - 0.001)" },
    ],
  },
  {
    id: "buy-feature-cancel-vs-confirm",
    name: "Buy feature — confirmation dialog cancel vs confirm",
    description: "Open the Buy Feature confirmation dialog, cancel it, and verify no charge or spin is triggered.",
    category: "buy_feature",
    severity: "major",
    requiresFeature: "buyBonus",
    setup_instructions:
    ...
    spin_count: 0,
    custom_assertions: [
      { id: "cancel-no-spin", description: "canceling buy confirmation triggers no spin response", check_code: "collector.spins.length === 0" },
      { id: "cancel-no-errors", description: "canceling buy confirmation produces no engine errors", check_code: "warnings.filter(w => /error|exception|threw/i.test(w)).length === 0" },
    ],
  },
  {
    id: "buy-feature-blocked-insufficient-funds",
    name: "Buy feature — blocked on insufficient funds",
    description: "When the wallet cannot afford the buy price, the purchase must be rejected without deduction.",
    category: "buy_feature",
    severity: "critical",
    requiresFeature: "buyBonus",
    setup_instructions:
    ...
    spin_count: 0,
    custom_assertions: [
      { id: "insufficient-buy-no-feature-spin", description: "insufficient-funds buy attempt does not start a feature spin", check_code: "collector.spins.length === 0" },
      { id: "no-negative-balance", description: "no observed balance is negative", check_code: "collector.spins.every(s => typeof s.endingBalance !== 'number' || s.endingBalance >= -0.01)" },
    ],
  },
  {
    id: "buy-feature-disabled-when-ante-on",
    name: "Buy feature — disabled when Ante ON",
    description: "With Ante ON, verify Buy Feature is unavailable or rejected without charge when the game rules require it.",
    category: "buy_feature",
    severity: "major",
    ...
    spin_count: 0,
    custom_assertions: [
      { id: "ante-on-buy-no-spin", description: "Buy under Ante ON inspection does not trigger a spin", check_code: "collector.spins.length === 0" },
      { id: "ante-on-buy-no-errors", description: "Buy under Ante ON inspection produces no engine errors", check_code: "warnings.filter(w => /error|exception|threw/i.test(w)).length === 0" },
    ],
  },
  {
...
function substituteCodeTokens(value: string, tokens: TemplateTokens): string {
  return value
    .replace(/\{\{betMin\}\}/g, tokens.betMin != null ? String(tokens.betMin) : "null")
    .replace(/\{\{betMax\}\}/g, tokens.betMax != null ? String(tokens.betMax) : "null")
    .replace(/\{\{defaultBet\}\}/g, tokens.defaultBet != null ? String(tokens.defaultBet) : "null")
    .replace(/\{\{gameName\}\}/g, JSON.stringify(tokens.gameName ?? ""));
}

function registryKeys(uiMap: import("../registry/types.js").UiRegistry | null): string[] {
...
function inferFeatureFromRegistry(
  ...
  switch (feature) {
    ...
    case "buyBonus":
      return hasAnyKey(keys, [/^buy(?:Bonus|Feature)?Button(?:__|$)/i, /buy.*bonus/i, /buy.*feature/i]);
    case "extraBet":
      return hasAnyKey(keys, [/^anteButton(?:__|$)/i, /ante/i, /double.?chance/i, /extra.?bet/i, /bet.?boost/i]);
    case "turbo":
      return hasAnyKey(keys, [/^turboButton(?:__|$)/i, /turbo/i, /quick.?spin/i, /fast.?spin/i]);
    case "history":
      return hasAnyKey(keys, [/^historyButton(?:__|$)/i, /history/i, /rounds/i]);
    case "paytable":
      return hasAnyKey(keys, [/^paytableButton(?:__|$)/i, /paytable/i, /rules/i, /info/i]);
    default:
      return false;
  }
}
...
function hasFeature(
  ...
  return Boolean(reg?.features?.[feature]?.present) || inferFeatureFromRegistry(uiMap, feature);
}

function extractNumericSuffix(key: string): number | null {
  const m = key.match(/(?:totalBet|betAmount|bet|chip|stake)-([0-9]+(?:[.,][0-9]+)?)/i);
  if (!m) return null;
...
function deriveBetTokensFromRegistry(
  ...
  const values = Array.from(new Set(registryKeys(uiMap).map(extractNumericSuffix).filter((n): n is number => n != null))).sort((a, b) => a - b);
