...
const actionUiKey = (a: CaseAction): string =>
  (a.kind === "click" || a.kind === "hold") ? a.uiKey : "";
...
export function detectUiOnlyCase(actions: CaseAction[]): {
  ...
  const hasOpen = actions.some((a) => (a.kind === "click" || a.kind === "hold") && isOpenUiKey(actionUiKey(a)));
  const hasClose = actions.some((a) => (a.kind === "click" || a.kind === "hold") && isCloseUiKey(actionUiKey(a)));
  ...
  const lastUiAction = [...actions].reverse().find((a) => a.kind === "click" || a.kind === "hold");
  const endsOnReopen = Boolean(lastUiAction && isOpenUiKey(actionUiKey(lastUiAction)));
