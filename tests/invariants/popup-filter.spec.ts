...
//   2. Coord matching a main key but with popup-specific label → KEPT (real
//      modal foreground buttons can be drawn over main controls).
//   3. Coord matching a sub-state key (has `__`) → KEPT (legit cross-popup
//      reuse: betMinus / betPlus both open the same bet selector).
//   4. Beyond tolerance → kept.
...
test("drops popup element overlapping a canonical main key (within tolerance)", () => {
  ...
  const r = filterMainOverlap([{ key: "spinButton_dup", x: 960, y: 600 }], REG);
  expect(r.kept).toHaveLength(0);
  expect(r.dropped).toHaveLength(1);
  expect(r.dropped[0]!.overlapsMainKey).toBe("spinButton");
});

test("keeps popup-specific foreground button even when it shares a main-control coord", () => {
  // QA feedback regression: real popup buttons may be drawn over the same
  // canvas area as a main button. Coordinate overlap alone must not drop them.
  const r = filterMainOverlap([{ key: "startButton", x: 960, y: 600 }], REG);
  expect(r.kept.map((x) => x.key)).toEqual(["startButton"]);
...
test("drops at tolerance boundary; keeps just outside", () => {
  ...
  const within = filterMainOverlap(
    [{ key: "spinButtonCandidate", x: 960 + (tol - 1), y: 600 }], REG,
  );
...
test("undefined entries in registry are ignored (no false drop)", () => {
  ...
  const r = filterMainOverlap([{ key: "betPlusCandidate", x: 1100, y: 600 }], reg);
...
test("non-finite coords on main entries are skipped (no NaN comparisons)", () => {
  ...
  const r = filterMainOverlap([{ key: "menuButtonCandidate", x: 100, y: 100 }], reg);
